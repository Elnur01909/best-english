...
| `BattleChallengePopup` əlavə edildikdən sonra `/friends` səhifəsi "Application error: a client-side exception has occurred" ilə tam çökürdü | Qlobal popup və `/friends` səhifəsi eyni anda eyni adlı (`incoming-battles:${userId}`) Supabase Realtime kanalına abunə olmağa çalışırdı — kitabxana 2-ci abunəliyə icazə vermir, "cannot add postgres_changes callbacks ... after subscribe()" exception atır → React render çökür | `subscribeToIncomingBattles()` hər çağırışda unikal kanal adı yaradır (`...:${random}`) → [[Dostlar-və-Yarış-Funksiyası]] |
